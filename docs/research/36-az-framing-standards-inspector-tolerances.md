...
| Should paint thresholds move to the NAHB 3/8 inch? | **Not as the production color.** That gauge is a 32-inch warranty check on the face of the wall. It is a different measurement than an 8 foot chord. Changing ε or the yellow-while-unlocked lock would mix a consumer booklet into a device-error budget that doc 33 still treats as unlocked. Doc 31 now **reports** model error against both this warranty gauge and the Handbook 0.11937° sketch. That report does not replace the yellow production paint. |
