...
`gen_wall_corner.py` writes a synthetic outside corner (two planes, optional floor, 5 mm spacing, 1–5 mm noise, leans planted at 0.383° and 0.250°). The PLY stays gitignored. The manifest is `data/wall-corner/manifest.json`. `infer_neural_corner.py --stack pointcept|randlanet --weights control|stud_finetune` scores ranks 4 and 5 into `artifacts/scorecards/phase2_neural_corner/`. That pass is wall faces, not studs. The note is `docs/research/23-phase2-neural-corner.md`.
