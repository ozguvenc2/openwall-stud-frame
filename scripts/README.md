...
`run_one_stud_five_finders.py` runs Open3D, PCL, CloudCompare RANSAC-SD, Pointcept, and Open3D-ML on one synthetic 2×4 (lean 0.05°, seed 2) and stops. It writes `docs/research/16-one-stud-five-finder-run.md`, one scorecard per finder, and the figures under `docs/research/images/one-stud-five-finders/`. A blocked install keeps metrics null.
