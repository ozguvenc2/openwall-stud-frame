...
| C3 | Floor-normal angle and gravity plumb are different references. Replacing the vector repaints the same boxes. | **Illustrated on paint, not on a stud.** Stage 0 and S1 use generator +Z. Stages 2 and 3 use a floor normal. Phase 2 measures the same painted planes against export +Z (1.005° and 0.763°) and against a fitted floor normal (1.685° and 0.976°) and sets both beside a SKIL level. The floor-normal leans are farther from the level. That is not an inclinometer paired to a stud axis. |
...
**E-P2 (2026-09-25, Oz_PC).** Painted outside wall corner, not a stud. SKIL digital level, three heights on each face. Open3D 0.20.0 planes on `data/raw/polycam/wall_corner_2026-09-25.ply` (gitignored; 2,560,150 points). The plumb proxy is the normal of a floor plane fit to the lower horizontal points. Export +Z is the before column on the same wall planes. Write-up: `docs/research/22-phase2-wall-corner-field.md`. Scorecard: `artifacts/scorecards/phase2_wall_corner/`, including `floor_gravity.json`. Python 3.12.10. This row does not run the six finders and does not claim a stud.
...
The cloud is a Polycam Custom close export, Z-up in the same sense as the Lot62 loft exports (floor near the bottom of Z). Open3D plane RANSAC on a 4 mm voxel downsample, distance 10 mm, then an SVD refit within 10 mm on the full cloud, returns two vertical planes. Plane 1 has 941,452 inliers (RMSE 5.0 mm). Plane 2 has 848,857 inliers (RMSE 3.4 mm). The angle between the normals is 89.72°.

The plumb proxy is a second fit, on the lower horizontal points only. The bottom 80 mm of export Z is voxel-downsampled at 4 mm. Normals are estimated in a 30 mm radius. Points with `|n_z| ≥ 0.94` are kept, and the lowest z-group of those points (gap 20 mm) is the slab. Open3D RANSAC at 5 mm, then an SVD refit within 5 mm, drops points already on either wall. The normal is flipped so it points from the floor center toward the mean of the two wall centers: into the room, away from the floor. On this export that normal has positive z. Lean from plumb is `arcsin(|n_wall · u|)` in degrees. Zero means the wall normal lies in the horizontal plane perpendicular to `u`, so the plumb line lies in the wall. The same number is the angle between the wall normal and that horizontal plane. Before, `u` is export +Z. After, `u` is this floor normal. The after column is the result.

The floor patch has 201,263 inliers, RMSE 1.6 mm, and an in-plane extent of 0.76 m × 0.66 m. It is 0.712° from export horizontal. Against export +Z the walls lean 1.005° and 0.763°. Against the floor normal they lean 1.685° and 0.976°. The corner edge is 1.259° from +Z and 1.943° from the floor normal. A looser 10 mm sequential floor, and a refit that drops points within 50 mm of either wall, move those floor-normal leans by about 0.1°. Those checks are not the result. An earlier card that used export +Z as the reference reported 1.003° and 0.763°. Open3D plane RANSAC is unseeded. This section quotes the card in which both columns share the wall planes.

Equal-count thirds along the floor normal are not the level’s contact points. Plane 1 reads 1.043° (bottom), 2.162° (mid), and 0.762° (top). Plane 2 reads 1.260°, 1.075°, and 0.638°. The cloud does not reproduce the SKIL top-to-bottom pattern. The close capture does not show the doorway or the cat tree, so the planes are not assigned to Face A or Face B. Sorting the two floor-normal leans against the two face means gives unpaired absolute gaps of 0.726° and 1.302°. The same sort on the export-+Z leans gives 0.513° and 0.622°. Both floor-normal leans are larger than both SKIL means, and larger than the export-+Z leans of the same planes. That gap is several times τ ≈ 0.119° and several times the display step. It is not a QA call. The floor normal did not close the gap to the level.
...
**Table 10. SKIL versus cloud lean from the floor normal (phase-2 pilot, not class F).** Cloud columns are equal-count thirds along the floor normal, plus the global plane. Lean is `arcsin(|n · u|)` with `u` the floor normal. They are not registered to the SKIL faces.
...
| Cloud plane 1, floor normal | 0.762° | 2.162° | 1.043° | 1.685° |
| Cloud plane 2, floor normal | 0.638° | 1.075° | 1.260° | 0.976° |

**Table 10b. Same wall planes, two up vectors (not class F).** Faces are still not registered. Gaps sort the two cloud leans against the two SKIL means (0.250° and 0.383°) and do not name a face.

| Up vector | Plane 1 | Plane 2 | Unpaired gaps to the SKIL means |
| --- | ---: | ---: | --- |
| Export +Z (before) | 1.005° | 0.763° | 0.513° and 0.622° |
| Floor normal (after) | 1.685° | 0.976° | 0.726° and 1.302° |
...
Phase 2 is that contact, on the wrong object for a stud claim. A painted corner with a level and two planes is a plumb pilot. The level faces are not straight (spreads 0.80° and 0.50°). The same planes lean 1.005° and 0.763° from export +Z and 1.685° and 0.976° from a floor normal used as up. They are not tied to a named face. The unpaired gaps against the floor normal, 0.726° and 1.302°, are larger than the export-+Z gaps and sit well outside τ and outside the 0.05° display step. The floor patch is short, it may not be level, and it is not a gravity instrument. Density is the other lesson. A room export at 102 mm or 65 mm nearest-neighbor median is not a stud surface. A close Custom export at 0.49 mm raw median, or about 4 mm if the cloud is read as occupied 4 mm cells (180,814 of them), is dense enough to sample a face and still is not the wood behind the paint. ε stays unlocked.
...
Phase 2 adds a painted-corner plumb pilot and a synthetic corner for the neural ranks, not a stud result. A SKIL level on two finished faces (means 0.383° and 0.250°) and two Open3D planes on a close cloud do not match. Against export +Z those planes lean 1.005° and 0.763°. Against a floor normal used as up they lean 1.685° and 0.976°. The faces were not registered. Room-scale exports of the same loft remain sparse. The close export is millimeter-class on paint. Paint is not a stud, class F is still empty, and the color stays yellow while ε is unlocked.
